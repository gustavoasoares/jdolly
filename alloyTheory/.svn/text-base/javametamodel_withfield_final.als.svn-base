...
//	all c: Class, f1,f2:c.fields | f1!=f2 =>f1.fid != f2.fid
    no c:Class | some disj f1,f2:Field | f1.id = f2.id && f1 + f2 in c.fields
//	all c: Class | all  f1,f2:c.fields | f1.id != f2.id
}

pred doNotReduceMethodVisibility[] {
 // all m1,m2 | one disj c1,c2:Class | c1 in c2.extend && m1 in c1.methods && m2 in c2.methods && m1.id = m2.id && m1.param = m2.param
 //              => 
}
...
pred noCalltoUndefinedField[] {
 all mi:FieldInvocation | 
  ...
  //all fi:FieldInvocation | fi.qField = this_ => 
     //                     (some c1: Class, m1:Method, f1:Field | 
        //                        fi in m1.b && f1.fid = fi.idField && m1 in c1.methods && f1 in c1.fields) //&& (c1 = c2)
           //               || (some disj c1,c2: Class, m1:Method, f1:Field | no f2:Field | 
              //                 fi in m1.b && f1.fid = fi.idField && m1 in c1.methods && f1 in c2.fields && c2 in c1.^extend && f1.facc != private_
                 //                   && f2 in c1.fields && f2.fid = f1.fid) //&& (c1 = c2)
}
